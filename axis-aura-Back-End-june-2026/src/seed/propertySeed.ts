...
/**
 * NOTE ON IMAGES:
 * These use free/open remote image URLs (Unsplash). They are only placeholders
 * so the site has real-looking content. Replace them later by re-uploading via
 * the dashboard so they are stored as Cloudinary URLs in the database.
 */
...
type SeedPaymentEntry = { heading: string; subText: string };
...
type SeedProperty = {
  ...
  images: string[];
  brochureFile: string;
  paymentPlans: Record<string, SeedPaymentEntry[]>;
  faqs: SeedFaq[];
  quarter: string;
  year: string;
  amenities: SeedFeature[];
  access: SeedFeature[];
  views: SeedFeature[];
  description: string;
  overview: string;
  ...
  floors: Record<string, {
    ...
    unitImages: Record<string, unknown>;
  }>;
};
...
const layoutImg = (id: string) =>
  `https://images.unsplash.com/photo-${id}?auto=format&fit=crop&crop=entropy&w=1000&h=750&q=80`;
...
const pick = <T,>(pool: T[], start: number, count: number): T[] => {
  const out: T[] = [];
...
const feat = (title: string, icon: string): SeedFeature => ({
...
const residentialAmenities: SeedFeature[] = [
...
const commercialAmenities: SeedFeature[] = [
...
const luxuryAmenities: SeedFeature[] = [
...
const accessCommon: SeedFeature[] = [
...
const viewsResidential: SeedFeature[] = [
...
const luxuryPaymentPlan: Record<string, SeedPaymentEntry[]> = {
  'Payment Plan': [
...
async function run() {
  ...
  console.log(
    `Property seed complete. Created: ${created}, Updated: ${updated}, Total: ${properties.length} ` +
