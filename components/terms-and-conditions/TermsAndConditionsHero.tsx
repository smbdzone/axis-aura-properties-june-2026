...
import { termsAndConditionsHero } from "@/components/data/termsAndConditions";

export default function TermsAndConditionsHero() {
  const { title, image, imageAlt } = termsAndConditionsHero;

  return (
    <section
      aria-labelledby="terms-conditions-hero-heading"
    ...
    >
      <div className="relative z-[3] w-full drop-shadow-[0_0_190px_rgba(0,0,0,0.25)]">
        <div className="relative flex min-h-[222px] flex-col overflow-hidden border-[1.5px] border-accent-light bg-white lg:block lg:h-[222px]">
          ...
          >
            <h1
              id="terms-conditions-hero-heading"
              className="text-center font-heading text-[clamp(1.5rem,5.5vw,56px)] font-bold leading-[150%] text-white"
            >
              {title}
            </h1>
          </div>
