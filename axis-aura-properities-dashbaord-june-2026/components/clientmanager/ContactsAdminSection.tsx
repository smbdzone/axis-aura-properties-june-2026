...
const gridClassName =
  "grid w-full grid-cols-[43px_110px_minmax(140px,1fr)_120px_120px_minmax(160px,1.4fr)_90px_48px] items-center gap-5";
...
function ContactTableRow({
  ...
  const displayName = item.fullName?.trim() || item.email || "—";
...
export default function ContactsAdminSection() {
  ...
  const sortedItems = useMemo(
    () =>
      applyDatedNameSort(
        items,
        sortBy,
        (item) => item.fullName?.trim() || item.email || "",
  ...
  const requestDeleteSelected = () => {
    if (!selectedRowIds.length) {
      toast.error("Select at least one message to delete.");
      return;
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      if (deleteDialog.type === "single") {
        ...
        toast.success(`Message from "${deleteDialog.label}" deleted.`);
      } else {
        await bulkDeleteContacts(selectedRowIds);
        setSelectedRowIds([]);
        toast.success(`${deleteDialog.count} message(s) deleted.`);
      }
      await loadContacts();
    } catch {
      toast.error("Failed to delete message(s). Please try again.");
    } finally {
      setDeleting(false);
  ...
  const deleteDialogTitle =
    deleteDialog?.type === "single" ? "Delete Message" : "Delete Messages";
  const deleteDialogMessage =
    deleteDialog?.type === "single"
      ? `Are you sure you want to delete the message from "${deleteDialog.label}"? This action cannot be undone.`
      : `Are you sure you want to delete ${deleteDialog?.count ?? 0} selected message(s)? This action cannot be undone.`;
          ...
          >
            <div className="border-b-[1.5px] border-accent-light px-6 pb-4">
              <div className={gridClassName}>
                ...
                <span className="text-xs text-black/60">Subject</span>
            ...
            <div className="flex flex-col gap-1">
              {loading ? (
                <p className="px-6 py-4 text-sm text-black/60">Loading messages...</p>
              ) : error ? (
                <p className="px-6 py-4 text-sm text-red-600">{error}</p>
              ) : paginatedItems.length === 0 ? (
                <p className="px-6 py-4 text-sm text-black/60">No contact messages yet.</p>
              ) : (
                paginatedItems.map((item, index) => (
                  <ContactTableRow
                    ...
                    onDelete={() =>
                      requestDeleteOne(item._id, item.fullName?.trim() || item.email || "—")
                    }
                  />
                ))
              )}
